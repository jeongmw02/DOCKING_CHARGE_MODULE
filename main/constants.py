...
SERVO_STOWED_ANGLE   = 0       # 대기/분리 위치 (deg)
...
# ── 모터 스텝 & 속도 ─────────────────────────────────────────
MOTOR_EXTENDED_STEPS = 11000  # 완전 연장 위치 (PRE_DOCKING / SEPARATION)
# 도킹 시: 11000 → 0,  분리 시: 0 → 11000

MOTOR_MIN_DELAY   = 0.001   # 최고 속도 (s/step)
MOTOR_MAX_DELAY   = 0.002   # 최저 속도 — 가감속 구간 (s/step)
MOTOR_ACCEL_STEPS = 300     # 가감속 구간 스텝 수

# ── 도킹 조건 ────────────────────────────────────────────────
SOFT_CAPTURE_DIST_MM = 300   # PRE_DOCKING 전이 ToF 임계값 (mm)
SOFT_CAPTURE_HOLD_S  = 2.0   # PRE_DOCKING 전이 지속 시간 (s) — ToF & ArUco 동시
HARD_LOCK_DIST_MM    = 100    # SOFT_CAPTURE → HARD_LOCK ToF 임계값 (mm)
